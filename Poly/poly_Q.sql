...
function  resequence  (p_polynome in polynome_row_ty, p_return in integer default null) return polynome_row_ty;
...
--
-- Elimate gaps and add fractions of the same power. Function can change the ID!
--
function resequence  (p_polynome in polynome_row_ty, p_return in integer default null) return polynome_row_ty
is
...
begin
  ...
  then
	l_id       := save_temp_polynome (p_polynome, l_id);
...
--
-- Delete element from an array with index n.
--
function  delete_element (p_polynome in polynome_row_ty, p_position in integer default 1, p_return in integer default null) return polynome_row_ty
is
l_polynome polynome_row_ty;
...
--
-- Calculate function result for a value.
--
function  result_for_x (p_polynome in polynome_row_ty, p_numerator in integer, p_denominator in integer) return types_pkg.fraction_ty
is
...
--
-- Load a polynome from the table with option to assign result to a new ID. Groups factors.
--
function load_polynome (p_id in integer, p_return in integer default null) return polynome_row_ty
is
l_polynome   polynome_row_ty;
...
--
-- Load a polynome from the temp table with option to assign the result to a new ID. Groups factors.
--
function  load_temp_polynome (p_id in integer, p_return in integer default null) return polynome_row_ty
is
l_polynome   polynome_row_ty;
...
l_polynome polynome_row_ty;
begin
  return add_polynomes (load_polynome (p_id1), multiply_polynome (p_id2, -1, 1, 0, new_id), p_result);
...
l_polynome polynome_row_ty;
begin
  return add_polynomes (p1_polynome, multiply_polynome (p2_polynome, -1, 1, 0, new_id), p_result);
...
--
-- Multiply 2  polynomes from a table using a Carthesian product.
--
function multiply_polynomes (p_id1 in integer, p_id2 in integer, p_result in integer) return polynome_row_ty
is
l_polynome polynome_row_ty;
...
begin
  ...
  while degree (l_remainder) >= l_divisor_deg
  loop
...
l_id_result   integer (8) := new_id (p_result);
begin
  if not is_empty (p_id1) and not is_empty (p_id2)
  then
...
--
-- Generate and return a random polynome.
--
function  random_polynome (p_result in integer default null, p_elements in integer default 10, p_min_max_factor in integer default 100, p_power in integer default 10) return polynome_row_ty
is
l_polynome polynome_Q_pkg.polynome_row_ty;
