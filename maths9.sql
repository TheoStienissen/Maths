...
function  bin2dec (p_value in varchar2) return integer;

function  dec2bin (p_value in integer) return varchar2;

function  bindigits (p_value in integer) return integer;
...
exception when others then
  util.show_error ('Error in function bin2dec. For value: ' || p_value || '.', sqlerrm);
end bin2dec;
...
then l_bin_string := '0';
  ...
  while l_value > 0
  loop
    l_bin_string := mod(l_value, 2) || l_bin_string;
...
exception when others then
  util.show_error ('Error in function dec2bin. For value: ' || to_char(p_value) || '.', sqlerrm);
end dec2bin;
...
if    p_value <= 1   then return 0;
elsif p_value <= 2   then return 1;
elsif p_value <= 4   then return 2;
elsif p_value <= 8   then return 3;
elsif p_value <= 16  then return 4;
else return round (ln (p_value)/ln (2));
end if;

exception when others then
  util.show_error ('Error in function bindigits for value: ' || p_value || '.', sqlerrm);
end bindigits;
...
begin
if   p_value1 = 0
then return abs (p_value2);
else return gcd (mod (p_value2, p_value1), p_value1);
end if;
...
then return -1;
else return p_value1 * p_value2 / gcd (mod (p_value2, p_value1), p_value1);
end if;
...
then return 0;
else return xgcd_first (p_value2,p_value1);
end if;
...
begin
  ...
  then
    ...
    return to_char (l_number) || ' *  ' || pfo (p_integer/l_number);
  end if;
...
--
-- People see odd numbers as male, even numbers as female
--
function odd (p_integer in integer) return boolean
is
begin
