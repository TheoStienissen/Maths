...
-- Location    : C:\Users\Theo\OneDrive\Theo\Project\Maths\Groups\groups1.7.sql
...
alter table cayley_lookup add constraint cayley_lookup_pk primary key (id);

create table cayley_ref_lookup
( id          integer generated always as identity
, lookup_id   number(6, 0)
, position    number(6, 0)
, int_code    number(6, 0));

alter table cayley_ref_lookup add constraint cayley_ref_lookup_pk primary key (id);
create unique index cayley_lookup_uk2 on cayley_lookup (g_order, string_code);
alter table cayley_ref_lookup add constraint cayley_ref_lookup_fk1 foreign key (lookup_id) references cayley_lookup (id) on delete cascade;
...
type cycle_ty is table of group_ty index by binary_integer;
...
procedure init (p_group in out group_ty, p_order in integer);

function  init (p_order in integer) return group_ty;
...
procedure show_header (p_comment in varchar2, p_type in varchar2, p_order in varchar2, p_fields in integer);

procedure print_cycles (p_cycle in cycle_ty);

function  present_in_cycle (p_value in integer, p_cycle in cycle_ty) return boolean;

function  permutation_to_cycles (p_group in group_ty) return cycle_ty;

function  cycles_to_group (p_cycle in cycle_ty, p_order in integer) return group_ty;
...
-- Ex: select permutation from table (group_pkg.show_permutations (7, 'RF'));
function  show_permutations (p_order in integer, p_type in varchar2) return MyPermutations_row pipelined
is 
l_char   varchar2(2);
begin 
...
elsif upper (p_type) = 'I' -- Only works if order <= 9
then 
...
-- Self explanatory. Fill group with identity element
procedure init (p_group in out group_ty, p_order in integer)
is
begin
  p_group.delete;
...
l_group  group_ty;
begin
  init (l_group, p_order);
  return l_group;

exception when others 
then 
  util.show_error ('Error in function init', sqlerrm);
end init;
...
l_result group_ty;
begin
  l_result(1) := p_group (p_group.count);
  for j in  1 .. p_group.count - 1
  loop 
    l_result (j + 1) := p_group (j);
  end loop;
  return l_result;
...
function  rf_group_to_string (p_group in group_ty) return varchar2
is
l_string   cayley_lookup.string_code%type;
l_count    integer := p_group.count;
l_dummy    integer;
l_match    boolean;
begin
 <<done>>
 for l in (select id, string_code from cayley_lookup where g_order = l_count)
 loop
   l_match  := true;
   l_string := l.string_code;
   for j in 1 .. 2
   loop
     select count(*) into l_dummy from cayley_ref_lookup where lookup_id = l.id and int_code = p_group (j) and position = j;
	 l_match := l_match and l_dummy = 1;	 
   end loop;
   exit done when l_match;   
  end loop;
 
  if l_match then return l_string; else return null; end if;
...
function  rf_string_order (p_string in varchar2, p_order in integer) return integer
is
l_string  cayley_lookup.string_code%type := p_string;
l_order   integer := 1;
begin
  <<done>>
  for n in 1 .. p_order
  loop
    ...
    if l_string = '1' then l_order := n; exit done; end if;
  end loop;
  return l_order;
...
-- Rotations
  for j in 1 .. p_order
  loop
    insert into cayley_lookup (g_order, string_code) values (p_order, nvl(substr (rpad ('R', j, 'R'), 2), 1)) returning id into l_id;
	for seq1 in 1 .. p_order
	loop
	  insert into cayley_ref_lookup (lookup_id, position, int_code) values  (l_id, seq1, l_group (seq1));
	end loop;
...
-- Flip + Rotations
  ...
  for j in 1 .. p_order
  loop
    insert into cayley_lookup (g_order, string_code) values (p_order, rpad('F', j, 'R')) returning id into l_id;
	for seq2 in 1 .. p_order
	loop
	  insert into cayley_ref_lookup (lookup_id, position, int_code) values (l_id, seq2, l_group(seq2));
	end loop;
	l_group := group_pkg.rf_rotate (l_group);
  end loop;
  commit;
...
begin 
--  rf_fill_cayley_lookup_table(p_order);
  <<done>>
...
-- Heading
  show_header ('Y*X', 'RF', p_order, 2 * p_order + 1);
...
-- Table
  ...
  loop
    ...
    loop 
	  ...
	  l_string := rf_group_to_string (l_group);
...
begin 
  ...
  loop 
    if mod (power (p_value, j), p_modulo) = 1 then l_order := j; exit done; end if;
  end loop;
  return l_order;
...
begin 
  ...
  for j in 1 .. p_modulo + 1
  loop 
    if mod (p_value * j, p_modulo) = 1 then l_inverse := j; exit done; end if;  
  end loop;
  return l_inverse;
...
begin 
-- Heading
  show_header ('M*', 'I', p_order, p_order + 1);
...
begin 
  ...
  for j in 1 .. p_modulo + 1
  loop 
    if mod (p_value + j, p_modulo) = 0 then l_order := j; exit done; end if;
  end loop;
  return l_order;
...
-- Formatting Output. 1 Function + 2 Procedures
function cell_padding (p_val in varchar2, p_order in integer, p_marker in varchar2, p_pad_char in varchar2 default ' ') return varchar2 
is 
begin
...
is
begin 
  print_cel (p_comment, p_order + 1);
  for h in (select permutation valh from table (group_pkg.show_permutations (p_order, p_type)))
  loop
    print_cel (h.valh, p_order);
  end loop;
  dbms_output.new_line;
  print_line (p_fields, p_order);
  
exception when others 
then 
  util.show_error ('Error in procedure show_header', sqlerrm);
end show_header;
...
is 
begin 
  for c in 1 .. p_cycle.count 
  loop
    dbms_output.put ('(');
    for g in 1 .. p_cycle(c).count - 1
	loop 
      dbms_output.put (to_char (p_cycle (c)(g), '990') || ' , ');
	end loop;
    dbms_output.put_line (to_char (p_cycle (c)(p_cycle(c).count), '990') || ')');
  end loop;
...
-- Check if a integer is already present in a cycle.
function  present_in_cycle (p_value in integer, p_cycle in cycle_ty) return boolean
is 
...
begin
  ...
  then
    ...
    for c in 1 .. p_cycle.count 
    loop
      for g in 1 .. p_cycle(c).count
	  loop 
        l_found := p_cycle (c)(g) = p_value;
...
-- Translate a group into 0 or more cycles.
function  permutation_to_cycles (p_group in group_ty) return cycle_ty
is
l_cycle       cycle_ty;
...
l_start       integer;
l_index       integer;
l_length      integer := p_group.count;
begin
for j in 1 .. l_length
loop
  if p_group (j) != j and not present_in_cycle (j, l_cycle)
  then
    l_start := p_group (j);
    l_cycle (l_cycle_count)(1) := l_start;
	l_pos := p_group (l_start);
	l_index := 1;
	while l_pos != l_start
	loop
	  l_index := l_index + 1;
	  l_cycle (l_cycle_count)(l_index) := l_pos;
	  l_pos   := p_group (l_pos);
	end loop;  
...
is
l_cycle cycle_ty;
l_group group_ty := init(p_order);
begin
  for c in 1 .. p_cycle.count 
  loop
    for g in 1 .. p_cycle(c).count - 1
    loop 
      l_group (p_cycle(c)(g)) := p_cycle(c)(g + 1);
	end loop;
    l_group (p_cycle (c)(p_cycle(c).count)) := p_cycle(c)(1);
  end loop;
  return l_group;

exception when others 
then 
  util.show_error ('Error in function cycles_to_group', sqlerrm);
end cycles_to_group;
...
create or replace package internal_group_pkg
accessible by (package group_pkg)
is
procedure mm_show_cayley_table (p_order in integer);
end internal_group_pkg;
/
