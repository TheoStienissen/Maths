...
Link:   @C:\Users\Theo\OneDrive\Theo\Project\Maths\Matrix\Matrix_Q_new.sql
-- N x M Matrix:
-- ( 11   ...    1m )
--        ...
-- ( n1   ...    nm )
...
as
zero constant    types_pkg.fraction_ty := fractions_pkg.to_fraction (0, 1);
one  constant    types_pkg.fraction_ty := fractions_pkg.to_fraction (1, 1);
...
function  to_point_Q_2D (p_x in types_pkg.fraction_ty, p_y in types_pkg.fraction_ty) return types_pkg.point_Q_ty_2D;

function  to_point_Q_3D (p_x in types_pkg.fraction_ty, p_y in types_pkg.fraction_ty, p_z in types_pkg.fraction_ty) return types_pkg.point_Q_ty_3D;
...
function  to_matrix_Q_2D (p_x11 in types_pkg.fraction_ty, p_x12 in types_pkg.fraction_ty, p_x21 in types_pkg.fraction_ty, p_x22 in types_pkg.fraction_ty) return types_pkg.matrix_Q_ty;
 
function  to_matrix_Q_3D (p_x11 in types_pkg.fraction_ty, p_x12 in types_pkg.fraction_ty, p_x13 in types_pkg.fraction_ty, p_x21 in types_pkg.fraction_ty, p_x22 in types_pkg.fraction_ty,
...
function  multiply    (p_matrix_a in types_pkg.matrix_Q_ty, p_matrix_b in types_pkg.matrix_Q_ty) return types_pkg.matrix_Q_ty;
...
exception when others then
  ...
  util.show_error ('Error in function to_point_Q_2D.', sqlerrm);
...
exception when others then
  ...
  util.show_error ('Error in function to_point_Q_3D.', sqlerrm);
...
exception when others then
  util.show_error ('Error in function to_matrix_Q_3D.' , sqlerrm);
  return constants_pkg.empty_matrix;
end to_matrix_Q_3D;
...
begin
  ...
  loop
    ...
    loop
      l_val := fractions_pkg.to_fraction (0, 1);    
...
function  mirror_xy   (p_point in types_pkg.point_Q_ty_3D) return types_pkg.point_Q_ty_3D
is
l_point types_pkg.point_Q_ty_3D;
begin
...
function  determinant (p_matrix in types_pkg.matrix_Q_ty) return types_pkg.fraction_ty
is
l_determinant types_pkg.fraction_ty := zero;
begin
...
l_matrix types_pkg.matrix_Q_ty;
begin
  for j in (select n, m, val_numerator, val_denominator from matrix_Q where id = p_id)
  loop
    l_matrix (j.n)(j.m) := fractions_pkg.to_fraction (j.val_numerator, j.val_denominator);
  end loop;
  return l_matrix;
...
l_matrix types_pkg.matrix_Q_ty;
begin
  for j in (select n, m, val_numerator, val_denominator from matrix_Q where name = p_name)
  loop
    l_matrix (j.n)(j.m) := fractions_pkg.to_fraction (j.val_numerator, j.val_denominator);
  end loop;
  return l_matrix;
...
begin
  ...
  loop
    ...
    loop
      begin
        insert into matrix_Q (id, n, m, val_numerator, val_denominator) values (p_id, n1, m1, p_matrix (n1)(m1).numerator, p_matrix (n1)(m1).denominator);
      exception when dup_val_on_index
      then
        update matrix_Q set val_numerator =  p_matrix (n1)(m1).numerator, val_denominator = p_matrix (n1)(m1).denominator where id = p_id and n = n1 and m = m1;
      end;
    end loop;
  end loop;
  commit;
...
function  dotproduct (p_vector1 in types_pkg.vector_Q_ty, p_vector2 in types_pkg.vector_Q_ty) return types_pkg.fraction_ty
is
l_dotproduct   types_pkg.fraction_ty := zero;
begin
...
begin
  ...
  loop
	...
	loop
	  l_matrix (n)(m) := fractions_pkg.multiply (
	                        fractions_pkg.to_fraction (power (-1, n + m) , 1)
							, determinant (cofactor (p_matrix, n, m)));
	end loop;
  end loop;
...
function  trace (p_matrix in types_pkg.matrix_Q_ty) return types_pkg.fraction_ty
is
l_sum  types_pkg.fraction_ty := zero;
begin
  if p_matrix (1).count != p_matrix.count
  then
...
begin
  ...
  return  multiply (fractions_pkg.divide (one, l_determinant), adjugate (p_matrix));
...
begin 
 ...
 -- dbms_output.put_line ('After subtract');
