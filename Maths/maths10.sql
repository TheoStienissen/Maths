Doc
  ...
  @C:\Users\Theo\OneDrive\Theo\Project\Maths\Maths\maths10.sql
...
function  mxgcd  (p_value1 in integer, p_value2 in integer) return types_pkg.matrix_Q_ty;

function  inverse (p_n in integer, p_m in integer) return integer result_cache;
...
function  nmod (p_n in integer, p_m in integer) return integer;
...
as
g_string     constant varchar2 (36) := '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
--
-- Binary to decimal
--
function bin_to_decimal (p_value in varchar2) return integer
...
--
-- Convert an integer from base 10 to another base. Allowed values for the base: 2 -- 36
--
function decimal_to_new_base (p_value in integer, p_base in integer) return varchar2
is
l_value      integer (38) := p_value;
...
begin
  if    nvl(p_value, 0) = 0 then l_result := to_char (p_value);
    ...
    while l_value > 0
    loop
      l_result := l_result || substr (g_string, mod (l_value, p_base) + 1, 1);
...
l_result     integer (38) := 0;
begin
  if p_value is not null and rtrim (p_value, substr (g_string, 1, p_base)) is null and p_base between 2 and 36
  then
    for j in 1 .. length (p_value)
    loop
      l_result := p_base * l_result + instr (g_string, substr (p_value, j, 1)) - 1;
    end loop;
  else raise_application_error (-20001, 'Invalid parameter values.');
  end if;
  return l_result;
...
is
begin
  if    p_value not between 0 and 34 then raise_application_error (-20001, 'Value for calculating nfac is out of range: 1 - 34.');
...
begin
  if p_value not between 0 and 25 then raise_application_error (-20001, 'Value for calculating pfac is out of range.');
  else
    check_init;
    for j in 1 .. p_value
    loop
      l_result := l_result * g_prime_tab (j);
    end loop;
  end if;
  return l_result;
...
is
begin
  if p_value1 = 0 then return abs (p_value2); else return gcd (nmod (p_value2, p_value1), p_value1); end if;
...
is
begin
  if nvl (p_value1, 0) = 0 or nvl (p_value2, 0) = 0 then return null;
  elsif p_value1 = p_value2 then return 1;
  else return mxgcd (p_value1, p_value2) (2) (1).numerator;
  end if; 
...
begin
  ...
  else return mxgcd (p_value1, p_value2) (2) (2).numerator;
  end if;
 
exception when others then
  util.show_error ('Error in function xgcd_last for pair: ' || p_value1 || ', ' || p_value2 || '.', sqlerrm);
  return null;
end xgcd_last;
...
--
-- maths.gcd(x,y) = x * maths.xgcd_first(x,y) + y * maths.xgcd_last(x,y)
-- The second line of the matrix contains xgcd_first and xgcd_last
--
function  mxgcd  (p_value1 in integer, p_value2 in integer) return types_pkg.matrix_Q_ty
is 
l_factor integer;
l_matrix types_pkg.matrix_Q_ty := matrix_Q_pkg.I (2);
l_value1 integer := p_value1;
l_value2 integer := p_value2;
l_dummy  integer;
begin 
  while l_value1 != 0
  loop 
    l_factor := floor (l_value2 / l_value1);
    l_matrix := matrix_Q_pkg.multiply (matrix_Q_pkg.to_matrix_2D (fractions_pkg.to_fraction ( - l_factor), fractions_pkg.to_fraction (1), fractions_pkg.to_fraction (1), fractions_pkg.to_fraction (0)), l_matrix);
	l_dummy  := l_value1;
	l_value1 := l_value2 - l_factor * l_value1;
	l_value2 := l_dummy;
  end loop;
  return l_matrix;
  
exception when others then
  util.show_error ('Error in function mxgcd for pair: ' || p_value1 || ', ' || p_value2 || '.', sqlerrm);
...
--
-- Euler:  a ** phi(b) = 1 (mod b), so a * a ** (pm - 2) = 1 (mod p_m)
--
function inverse (p_n in integer, p_m in integer) return integer result_cache
is 
begin 
  if maths.gcd (p_n, p_m) != 1 then raise_application_error (-20001, 'Parameter values are not relatively prime.');
  elsif is_prime (p_m)
  then return maths.powermod (p_n, p_m - 2, p_m);
  else return maths.powermod (p_n, maths.phi (p_m) - 1, p_m);
  end if;	

exception when others then
  util.show_error ('Error in function inverse for pair: ' || p_n || ', ' || p_m || '.', sqlerrm);
  return null;
end inverse;
...
l_sqrt        integer (19) := 5;
l_sqr         integer (38) := 25;
l_count       integer (8)  := 10;
l_nr          integer (20);
l_divisor     boolean;
begin
  g_prime_tab := prime_ty (1 => 2, 2 => 3, 3 => 5, 4 => 7, 5 => 11, 6 => 13, 7 => 17, 8 => 19, 9 => 23, 10 => 29);
  
  for n in 15 .. ceil (p_max / 2)
  loop
    l_nr := 2 * n + 1;
    if l_nr > l_sqr
    then
    ...
    for m in 1 .. l_count
    loop
      exit done when g_prime_tab (m) > l_sqrt;
      l_divisor := mod (l_nr, g_prime_tab (m)) = 0;
...
is
begin
  if maths.g_prime_tab.count = 0 then maths.init_primes; end if;
...
begin
  ...
  <<done>>
  for n in 1 .. g_prime_tab.count
  loop
    l_divisor := mod (p_integer, g_prime_tab(n)) = 0;
    exit done when l_divisor or g_prime_tab(n) * g_prime_tab(n) > p_integer;
  end loop;
...
--
-- Function to check whether a certain integer is a prime returning an integer. 1 = TRUE, 0 = FALSE
--
function is_prime_n (p_integer in integer) return integer
is
begin
...
begin
  ...
  for n in 1 .. g_prime_tab.count
  loop
    b_divisor := mod (p_integer, g_prime_tab (n)) = 0;
    if b_divisor then l_divisor := g_prime_tab (n); end if;
    exit done when b_divisor or g_prime_tab (n) * g_prime_tab (n) > p_integer;
  end loop;
  return l_divisor;
...
begin
  ...
  loop
    ...
    then
	  ...
	  l_no_divisors := case m * m when p_integer then l_no_divisors + 1 else l_no_divisors + 2 end;
    end if;
  end loop;
...
begin
  ...
  for n in 1 .. g_prime_tab.count
  loop
    while mod (l_integer, g_prime_tab (n)) = 0
    loop
      l_divisor   := true;
      l_occurence := l_occurence + 1;
      l_integer   := l_integer / g_prime_tab (n);
    end loop;
    if l_divisor
    then
      pipe row (pfo_row (g_prime_tab (n), l_occurence));
      l_occurence := 0;
      l_divisor   := false;
    end if;
...
is
begin
  return abs (nmod (p_integer, p_base));
...
begin
  ...
  while g_prime_tab (l_count) <= p_integer  -- g_prime_tab.count ??
  loop
      l_product := l_product * g_prime_tab (l_count);
	  l_count   := l_count + 1;
  end loop;
  return l_product;
...
l_return integer;
begin
  select to_number (reverse (to_char (p_integer))) into l_return from dual; -- This is a different reverse function!!
  return l_return;
...
begin
  ...
  elsif p_power  = 1 then return nmod (p_n, p_mod);
  else return nmod (powermod (p_n, trunc (p_power/2), p_mod) * powermod (p_n, ceil (p_power/2), p_mod), p_mod);
  end if;
...
begin
  check_init;  
  return maths.g_prime_tab (trunc (dbms_random.value (p_low, p_high)));
...
begin
  ...
  return maths.nmod (l_result, maths.lcm (p_mod_n, p_mod_m));
...
l_m   integer := abs(p_m);
begin
  return p_n - floor (p_n/p_m) * p_m;
  
exception when others then
  util.show_error ('Error in function nmod for ' || p_n || ' and ' || p_m || '.', sqlerrm);
  return null;  
end nmod;
...
begin
  check_init;
  for j in 1 ..  maths.g_prime_tab.count
  loop
    pipe row (prime_row (j, g_prime_tab (j)));
  end loop;
...
l_count number(6) := 0;
begin
for j in 2 .. maths.g_prime_tab.last
loop
if maths.g_prime_tab (j) -  maths.g_prime_tab (j - 1) = 2
then
   l_count := l_count + 1;
   dbms_output.put_line (rpad(to_char(l_count, '99999'), 8) || ' #    ' || to_char(maths.g_prime_tab (j - 1)) || ' -  ' || to_char(maths.g_prime_tab (j)));
end if;
end loop;
end; 
/
...
for j in 1 .. 200
loop
  for a in 2 .. maths.g_prime_tab (j) - 1
  loop
  l_mod := maths.powermod (a, trunc(maths.g_prime_tab (j)/2), maths.g_prime_tab (j));
  if l_mod != 1 and l_mod != maths.g_prime_tab (j) -1
  then
    dbms_output.put_line(maths.g_prime_tab (j) || '  ' || a|| '  ' || maths.powermod (a, trunc(maths.g_prime_tab (j)/2), maths.g_prime_tab (j)));
  end if;
  end loop;
end loop;
end;
...
alter package MATRIX_Q_PKG compile body;

set serveroutput on size unlimited
declare
l_matrix types_pkg.matrix_Q_ty;
begin
  l_matrix := maths.mxgcd (245628,774471);
