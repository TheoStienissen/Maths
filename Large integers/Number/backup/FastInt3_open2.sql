...
begin
  ...
  then
    ...
    select count(*) into l_count from fast_int_cert_join where fast_int_id = l_id;
	if l_count = 0 then insert into fast_int_cert_join (fast_int_id, cert_id) values (l_id, l_cert_id); end if;
...
-- Load sqrt (N) into l_a
  ...
  then
    ...
    l_id := fast_int_pkg.save_number (l_a);
    update fast_int_cert_join set sqrt_id = l_id where fast_int_id = p_integer_id;
  else
...
begin
  fast_int_pkg.run_factoring_routine (188, p_iterations => 400, p_interval => 1000000);
end;
/
