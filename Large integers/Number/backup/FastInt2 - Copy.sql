...
procedure p_check_certificate (p_cert_id in integer, p_lower_bound in integer default null, p_interval integer default 100);
...
function is_square (p_number in types_pkg.fast_int_ty) return boolean
is
l_sqrt types_pkg.fast_int_ty := fast_int.fsqrt (p_number);
begin
  if    mod (l_sqrt (1) * l_sqrt (1), g_base) = p_number (1)
  then  return fast_int.eq (fast_int.multiply (l_sqrt, l_sqrt), p_number);
  else  return FALSE;
  end if;
...
begin
  begin 
    select distinct id into l_id from fast_int_tbl where ref_id = p_cert_id;
  ...
  then
	...
	l_id := save_number (l_N, p_ref_id  => p_cert_id);
  end;  
  if not odd (l_N) then raise_application_error (-20001, 'Prime product of large primes cannot be even!'); end if;

  -- Initial value for a
  begin 
    select distinct id into l_id from fast_int_tbl where ref_id = (select distinct id from fast_int_tbl where ref_id = p_cert_id);
  ...
  then select nvl (max (upperbound), 0) + 1 into l_lower_bound from certificate_tests where cert_id = p_cert_id;
  else l_lower_bound := p_lower_bound;
  end if;
  l_a := fast_int.add_n (l_lower_bound, l_a);
  insert into certificate_tests (cert_id, lowerbound, upperbound) values (p_cert_id, l_lower_bound, l_lower_bound + p_interval) returning id into l_id;
  commit;
  ...
  loop
    ...
    then
	  if is_square (l_b2)
	  then
        l_rc := fast_int.save_number (l_b2);
        update certificate_tests set confirmed = l_rc where id = p_cert_id;
	    if sql%notfound
	    then
	      insert into certificate_tests (cert_id, confirmed, lowerbound, upperbound) values (p_cert_id, l_rc, l_lower_bound, l_lower_bound + j - 1);
	    end if;
	  commit;
	  raise_application_error (-20001, 'Square root found for boundary: ' || l_lower_bound);
	  end if;
    end if;
...
begin
fast_int.p_check_certificate (109, 14000, 100);
end;
/
...
select * from certificate_tests where cert_id = 109 order by lowerbound;
...
begin
fast_int.p_check_certificate (208);
end
/

begin
  fast_int.run_certificate_test (109, 50);
end;
/
...
l_x integer;
begin 
fast_int.print(fast_int.load_number (209));
dbms_output. put_line ('----------------------------');
fast_int.print(fast_int.load_number (210));
--
end;
/


      4011        208              53000102   53100102         -1
      4008        208              53100102   53200102         -1
