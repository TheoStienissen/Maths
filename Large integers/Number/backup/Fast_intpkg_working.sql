...
-- @"C:\Users\Theo\OneDrive\Theo\Project\Maths\Large integers\Number\FastInt3_open.sql"
...
elsif fmodi (l_b2,  3) in (2)                                                                                            then return false;
...
elsif fmodi (l_b2, 10) in (2, 3, 7, 8)                                                                                   then return false;
elsif fmodi (l_b2, 11) in (2, 6, 7, 8, 10)                                                                               then return false;
elsif fmodi (l_b2, 12) in (2, 3, 5, 6, 7, 8, 10, 11)                                                                     then return false;
...
elsif fmodi (l_b2, 26) in (2, 5, 6, 7, 8, 11, 15, 18, 19, 20, 21, 24)                                                    then return false;
elsif fmodi (l_b2, 27) in (2, 3, 5, 6, 8, 11, 12, 14, 15, 17, 18, 20, 21, 23, 24, 26)                                    then return false;
elsif fmodi (l_b2, 28) in (2, 3, 5, 6, 7, 10, 11, 12, 13, 14, 15, 17, 18, 19, 20, 22, 23, 24, 26, 27)                    then return false;
elsif fmodi (l_b2, 29) in (2, 3, 8, 10, 11, 12, 14, 15, 17, 18, 19, 21, 26, 27)                                          then return false;
elsif fmodi (l_b2, 30) in (2, 3, 5, 7, 8, 11, 12, 13, 14, 17, 18, 20, 22, 23, 26, 27, 28, 29)                            then return false;
...
elsif fmodi (l_b2, 33) in (2, 5, 6, 7, 8, 10, 11, 13, 14, 17, 18, 19, 20, 21, 23, 24, 26, 28, 29, 30, 32)                then return false;
elsif fmodi (l_b2, 34) in (3, 5, 6, 7, 10, 11, 12, 14, 20, 22, 23, 24, 27, 28, 29, 31)                                   then return false;
elsif fmodi (l_b2, 35) in (2, 3, 5, 6, 7, 8, 10, 12, 13, 17, 18, 19, 20, 22, 23, 24, 26, 27, 28, 31, 32, 33, 34)         then return false;
else  return true;  
end if;
end validate_if_square;
--
begin
  -- First check if we are already done. If so, do nothing.
  ...
  then
    ...
    -- Initial value for square root a
    select count(*) into l_count from fast_int_cert_join where fast_int_id = p_integer_id;
	if l_count = 0 then insert into fast_int_cert_join (fast_int_id, cert_id) values (p_integer_id, l_cert_id); end if;

--  Load the square root
/*
	select max(sqrt_id) into l_id from fast_int_cert_join where fast_int_id = l_id;
	if l_id is null	
	then l_a       := fast_int_pkg.fsqrt (l_N, true);
         l_id := fast_int_pkg.save_number (l_a);
		 update fast_int_cert_join set sqrt_id = l_id where fast_int_id = l_id;
		 commit;
	else l_a       := fast_int_pkg.load_number (l_sqrt_id);
    end if;
*/
    l_a       := fast_int_pkg.fsqrt (l_N, true);
    l_first_b2 := fast_int_pkg.subtract (fast_int_pkg.multiply (l_a, l_a), l_N);
	
-- Load the initial value
/*
    select max(init_val) into l_id from fast_int_cert_join where fast_int_id = p_integer_id;
    if l_id is null	
    then
      l_first_b2 := fast_int_pkg.subtract (fast_int_pkg.multiply (l_a, l_a), l_N);
      l_id := fast_int_pkg.save_number (l_first_b2);
      update fast_int_cert_join set init_val = l_id where fast_int_id = p_integer_id;
	  commit;
	else l_first_b2  := fast_int_pkg.load_number (l_id);
    end if;
	commit;
*/
    ...
    then select nvl (max (upperbound), 0) + 1 into l_lower_bound from rsa_factoring where fast_int_id = l_id;
    else l_lower_bound := p_lower_bound;
    end if;	
	l_upper_bound := l_lower_bound + p_interval;
    insert into rsa_factoring (fast_int_id, confirmed, lowerbound, upperbound) values (l_id, -1, l_lower_bound, l_upper_bound);
    commit;
    ...
    loop
      ...
      then
	    ...
	    then  	
          ...
          update rsa_factoring set confirmed = l_rc where fast_int_id = l_id and upperbound = l_upper_bound;
	      l_rc := fast_int_pkg.save_number (l_a);
          insert into rsa_factoring (fast_int_id, confirmed, lowerbound, upperbound) values (l_id, l_rc, l_lower_bound, l_count);
...
--
-- Testing parallel at max capacity using the dbms_job package
--
procedure run_factoring_routine (p_integer_id in integer, p_iterations in integer default 100, p_interval integer default 10000000)
is 
...
l_job         integer;
l_cpus        pls_integer;
l_job_name    constant varchar2(30) := 'fast_int_pkg.p_factor_product';
l_job_name_length number(2);
l_id          integer;
l_N           types_pkg.fast_int_ty;
l_a           types_pkg.fast_int_ty;
begin
  l_job_name_length:= length (l_job_name);

  select count(*) into l_count from fast_int_cert_join where fast_int_id = p_integer_id;
  if l_count = 0 then insert into fast_int_cert_join (fast_int_id) values (p_integer_id); end if;
  

  l_N := fast_int_pkg.load_number (p_integer_id); -- Assume this product exists

-- Load sqrt (N) into l_a
  select max (sqrt_id) into l_id from fast_int_cert_join where fast_int_id = p_integer_id;
  if l_id is null	
  then
    update fast_int_cert_join set sqrt_id = fast_int_pkg.save_number (fast_int_pkg.fsqrt (l_N)) where fast_int_id = p_integer_id;
  else
    l_a := fast_int_pkg.load_number (l_id);
  end if;

  /*
-- Save initial value for a  
  select max (init_val) into l_id from fast_int_cert_join where fast_int_id = p_integer_id;
  if l_id is null	
  then
    l_id := fast_int_pkg.save_number (fast_int_pkg.subtract (fast_int_pkg.multiply (l_a, l_a), l_N));
    update fast_int_cert_join set init_val = l_id where fast_int_id = p_integer_id;
  end if;
  commit;
  */
  ...
  loop
    select count(*) into l_count from dba_jobs_running r, dba_jobs j where r.job = j.job and substr (j.what, 1, l_job_name_length) = l_job_name;
...
is 
begin 
  for j in (select job from dba_jobs)
  loop 
...
begin
  ...
  loop
    l_cnt     := l_cnt + 1;
    l_product := fast_int_pkg.mult_n (maths.p_prime_tab (l_cnt), l_product);
  end loop;
...
begin
  fast_int_pkg.p_factor_product (248);
end;
/
...
begin
  fast_int_pkg.run_factoring_routine (188, p_iterations => 1000, p_interval => 1000000);
end;
/
...
--begin
--  fast_int_pkg.p_factor_product (188, 14020, 10);
--end;
--/
