...
--
begin
    select /*+ RESULT_CACHE */ count (*) into l_count from fast_int_tbl where id = l_id;
	if l_count = 0
	then
	  l_cert_id := p_integer_id;
      select certificate into l_certificate from certificates where id = l_id;
      l_N  := fast_int_pkg.f_certificate_hex_to_dec (l_certificate);
	  l_id := fast_int_pkg.save_number (l_N); 	
	else
	  l_N := fast_int_pkg.load_number (l_id);
	end if;
    if not odd (l_N) then raise_application_error (-20001, 'Prime product of large primes cannot be even!'); end if;

    -- Initial value for square root a. Assume this exists
...
--  Load the square root
	select sqrt_id, init_val into l_sqrt_id, l_init_val from fast_int_cert_join where fast_int_id = l_id;
	if l_sqrt_id is null	
	then l_a       := fast_int_pkg.fsqrt (l_N, true);
         l_sqrt_id := fast_int_pkg.save_number (l_a);
		 update fast_int_cert_join set sqrt_id = l_sqrt_id where fast_int_id = l_id;
...
-- Load the initial value
    ...
    then
      ...
      update fast_int_cert_join set init_val = l_init_val where fast_int_id = l_id;
