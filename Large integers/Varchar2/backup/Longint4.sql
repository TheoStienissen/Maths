...
l_tmp   := multiply(power_mod(p_base, l_halve, p_mod), power_mod(p_base, l_halve, p_mod));

if l_rest  = '0'
then
  return long_int.mod(l_tmp, p_mod);
else
...
l_witness        integer;
l_power          integer := 0;
function witness (p_candidate in varchar2, p_remainder in varchar2, p_power in integer, p_witness in integer) return boolean
is
x      varchar2(32767);
n1     varchar2(32767);
b      boolean;
begin
x := long_int.power_mod (p_witness, p_remainder, p_candidate);
...
while eq(long_int.mod(l_remainder, '2'), '0')
loop
  l_remainder := divide(l_remainder, '2');
  l_power     := l_power + 1;
end loop;
...
select long_int.prime_test('8999176623527488915637849594853483485637383889507859048635435754398899917662357383782456247274889156378498', 1) from dual;
